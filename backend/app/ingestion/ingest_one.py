...
from app.ingestion.seeder import seed_sources_from_file, get_source_by_id, get_or_create_source
...
def ingest_feed_from_url(feed_url: str, source_id: Optional[int] = None) -> Dict[str, int]:
    """
            ...
            for entry in feed.entries:
                try:
                    ...
                    logger.warning(
                        ...
                        extra={"error": str(e), "entry_title": getattr(entry, 'title', 'Unknown')}
                    )
        ...
        try:
            with SessionLocal() as db:
                if 'source' in locals():
                    ...
                    source.last_error = str(e)[:1000]  # Truncate error message
                    db.commit()
        except Exception:
...
def ingest_feed_from_file(file_path: str) -> Dict[str, int]:
    ...
    try:
        ...
        source = get_or_create_source(f"file://{file_path}", f"Local File ({Path(file_path).name})")
        ...
        with SessionLocal() as db:
            ...
            try:
                ...
                db.rollback()
                logger.error("Database commit failed for file ingestion", extra={"error": str(e)})
                result["errors"] += result["inserted"]
                result["inserted"] = 0
                raise
...
Examples:
  ...
  python -m app.ingestion.ingest_one --file tests/fixtures/feeds/sample_feed.xml
    ...
    group.add_argument(
        ...
        type=str,
        metavar='URL',
        help='Ingest articles from feed URL'
    )
    ...
    group.add_argument(
        ...
        type=int,
        metavar='ID',
        help='Ingest articles from source by ID'
    )
...
def main() -> int:
    ...
    try:
        if args.seed_sources:
            ...
            print(f"Source seeding completed: {result['created']} created, {result['updated']} updated, {result['skipped']} skipped")

        elif args.feed_url:
            # Ingest from URL
            result = ingest_feed_from_url(args.feed_url)
            print(f"Feed ingestion completed: {result['parsed']} parsed, {result['inserted']} inserted, {result['skipped']} skipped, {result['errors']} errors")

        elif args.source_id:
            # Ingest from source ID
            with SessionLocal() as db:
            ...
            print(f"File ingestion completed: {result['parsed']} parsed, {result['inserted']} inserted, {result['skipped']} skipped, {result['errors']} errors")
